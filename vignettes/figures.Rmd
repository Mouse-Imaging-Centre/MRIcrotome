...
vignette: >
  %\VignetteIndexEntry{Making pretty figures using MRIcrotome}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r setup, include = FALSE}
# The example data (sliceData.RData) is not distributed with the package.
# The code runs only when the file is present; the pre-rendered figures.html
# in the repository shows the output.
knitr::opts_chunk$set(eval = file.exists("sliceData.RData"))
```
...
The best way to use MRIcrotome is through pipes; the `%>%` pipe comes from _magrittr_, which _purrr_ re-exports, and MRIcrotome already depends on _purrr_ (the full tidyverse works just as well). Also, the graphics subsystem is _grid_ graphics, so let's go ahead and load that as well.
