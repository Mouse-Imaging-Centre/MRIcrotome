test_that("low/high default to the 2nd/98th percentiles (#6, #19)", {
  ...
  expect_equal(li[["overlay#1"]]$low, 0)
  expect_equal(li[["overlay#1"]]$high, 2)
  # symmetric overlays take the range from the absolute values
  ssm <- sliceSeries(nrow = 1, begin = 5, end = 20) |> anatomy(v$anat, 0, 3000) |> overlay(v$stats, symmetric = TRUE)
  expect_equal(unlist(ssm$ssl[[1]]$legendInfo[["overlay#1"]][c("low", "high")]),
